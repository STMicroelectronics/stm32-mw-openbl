...
#include "openbl_itf_otp.h"

/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define OPENBL_OTP_DATA_SIZE                  0x02U  /* OTP half-word size in bytes */
#define OPENBL_OTP_PROGRAM_TIMEOUT            10U    /* Timeout value for writing half-word data size in OTP */

/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
/* Private function prototypes -----------------------------------------------*/
static void OPENBL_OTP_Program(openbl_mem_t *p_mem, uint32_t otp_address, uint8_t *p_data, uint32_t size_byte);
...
uint8_t OPENBL_OTP_Read(openbl_mem_t *p_mem, uint32_t address)
{
  ...
  if ((tmp_address % OPENBL_OTP_DATA_SIZE) != 0U)
  {
    index        = (tmp_address % OPENBL_OTP_DATA_SIZE);
    tmp_address -= index;
  }
...
{
  uint32_t index;
  __ALIGNED(4) uint8_t data[OPENBL_OTP_DATA_SIZE] = {0x0U};
  uint8_t remaining;
  ...
  if ((p_data != NULL) && (data_length != 0U))
  {
    ...
    while (data_length > OPENBL_OTP_DATA_SIZE)
    {
      for (index = 0U; index < OPENBL_OTP_DATA_SIZE; index++)
      {
        data[index] = *(p_data + index);
      }

      OPENBL_OTP_Program(p_mem, address, (uint8_t *)data, OPENBL_OTP_DATA_SIZE);

      address     += OPENBL_OTP_DATA_SIZE;
      p_data      += OPENBL_OTP_DATA_SIZE;
      data_length -= OPENBL_OTP_DATA_SIZE;
    }

    /* If remaining count, go back to fill the rest with 0xFF */
    if (data_length > 0U)
    {
      remaining = OPENBL_OTP_DATA_SIZE - data_length;
...
/**
  * @brief  Program OTP memory at specified address.
  * @param  p_mem Pointer to the memory object.
  * @param  otp_address Specifies the address where data will be programmed.
...
static void OPENBL_OTP_Program(openbl_mem_t *p_mem, uint32_t otp_address, uint8_t *p_data, uint32_t size_byte)
{
  ...
  /* Write data in OTP without checking for errors */
  (void)HAL_FLASH_OTP_ProgramByAddr(hflash,
                                    otp_address,
                                    (uint32_t *)p_data,
                                    size_byte,
                                    OPENBL_OTP_PROGRAM_TIMEOUT);
}
