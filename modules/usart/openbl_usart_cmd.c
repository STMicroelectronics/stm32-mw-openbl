...
static uint8_t OPENBL_USART_GetAddress(openbl_itf_obj_t *p_itf, openbl_mem_list_t *p_mem_list, uint32_t *Address);
static uint8_t OPENBL_USART_GetSpecialCmdOpCode(openbl_itf_obj_t *p_itf, openbl_special_cmd_type_t cmd_type,
                                                uint16_t *opcode);
...
{
  static openbl_commands_t usart_commands =
  {
  ...
  return (&usart_commands);
}

/**
  * @brief  This function is used to construct the command opcodes list.
  * @param  p_usart_cmd Pointer to the structure containing the USART supported commands.
  * @param  p_cmd_opcodes_list Pointer to the structure containing the list of supported commands by the USART protocol.
  * @return None.
  */
                                          ...
                                          openbl_cmd_opcodes_list_t *p_cmd_opcodes_list)
{
  ...
  if ((p_usart_cmd->GetCommand != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_GET_COMMAND;
    i++;
  }

  if ((p_usart_cmd->GetVersion != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_GET_VERSION;
    i++;
  }

  if ((p_usart_cmd->GetID != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_GET_ID;
    i++;
  }

  if ((p_usart_cmd->ReadMemory != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_READ_MEMORY;
    i++;
  }

  if ((p_usart_cmd->Go != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_GO;
    i++;
  }

  if ((p_usart_cmd->WriteMemory != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_WRITE_MEMORY;
    i++;
  }

  if ((p_usart_cmd->EraseMemory != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_EXT_ERASE_MEMORY;
    i++;
  }

  if ((p_usart_cmd->WriteProtect != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_WRITE_PROTECT;
    i++;
  }

  if ((p_usart_cmd->WriteUnprotect != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_WRITE_UNPROTECT;
    i++;
  }

  if ((p_usart_cmd->ReadoutProtect != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_READ_PROTECT;
    i++;
  }

  if ((p_usart_cmd->ReadoutUnprotect != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_READ_UNPROTECT;
    i++;
  }

  if ((p_usart_cmd->SpecialCommand != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
    p_cmd_opcodes_list->p_opcodes[i] = OPENBL_CMD_SPECIAL_COMMAND;
    i++;
  }

  if ((p_usart_cmd->ExtendedSpecialCommand != NULL) && (i < (uint8_t)OPENBL_USART_COMMANDS_MAX_NBR))
  {
...
void OPENBL_USART_GetCommand(openbl_itf_obj_t *p_itf)
{
  ...
  OPENBL_ITF_USART_SendAcknowledge(p_itf, OPENBL_ACK_BYTE);

  /* Send the number of commands supported by the USART protocol */
  OPENBL_ITF_USART_SendByte(p_itf, (uint8_t)(p_itf->cmd_opcodes_list.number));

  /* Send USART protocol version */
  OPENBL_ITF_USART_SendByte(p_itf, OPENBL_USART_VERSION);

  /* Send the list of supported commands */
  for (counter = 0U; counter < p_itf->cmd_opcodes_list.number; counter++)
  {
    OPENBL_ITF_USART_SendByte(p_itf, p_itf->cmd_opcodes_list.p_opcodes[counter]);
  }
...
void OPENBL_USART_GetID(openbl_itf_obj_t *p_itf)
{
  ...
  OPENBL_ITF_USART_SendByte(p_itf, 0x01U);

  /* Send the device ID starting by the MSB byte then the LSB byte */
  OPENBL_ITF_USART_SendByte(p_itf, OPENBL_DEVICE_ID_MSB);
  OPENBL_ITF_USART_SendByte(p_itf, OPENBL_DEVICE_ID_LSB);
...
void OPENBL_USART_ReadMemory(openbl_itf_obj_t *p_itf)
{
  ...
  {
    OPENBL_ITF_USART_SendAcknowledge(p_itf, OPENBL_NACK_BYTE);
  }
  else
  {
    OPENBL_ITF_USART_SendAcknowledge(p_itf, OPENBL_ACK_BYTE);
    ...
    if (OPENBL_USART_GetAddress(p_itf, p_itf->p_mem_list, &address) == OPENBL_NACK_BYTE)
    {
      OPENBL_ITF_USART_SendAcknowledge(p_itf, OPENBL_NACK_BYTE);
    }
    else
    {
      ...
      data = OPENBL_ITF_USART_ReadByte(p_itf);
      xor  = ~data;

      /* Check data integrity */
      if (OPENBL_ITF_USART_ReadByte(p_itf) != xor)
      {
        OPENBL_ITF_USART_SendAcknowledge(p_itf, OPENBL_NACK_BYTE);
      }
      else
      {
        ...
        {
          OPENBL_ITF_USART_SendByte(p_itf, OPENBL_MEM_Read(p_mem, address));
          address++;
        }
      }
    }
...
void OPENBL_USART_WriteMemory(openbl_itf_obj_t *p_itf)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (OPENBL_ITF_USART_ReadByte(p_itf) != tmpXOR)
      {
        OPENBL_ITF_USART_SendAcknowledge(p_itf, OPENBL_NACK_BYTE);
      }
      else
...
void OPENBL_USART_EraseMemory(openbl_itf_obj_t *p_itf)
{
  ...
  else
  {
    ...
    data = OPENBL_ITF_USART_ReadByte(p_itf);
    data = (uint16_t)(data << 8) | OPENBL_ITF_USART_ReadByte(p_itf);
    ...
    {
      /* Check data integrity */
      if (OPENBL_ITF_USART_ReadByte(p_itf) != (uint8_t) xor)
      {
    ...
    else
    {
      ...
      *p_tmp_buffer = (uint8_t)((numpage & 0xFF00U) >> 8);
      ...
      for (counter = numpage; counter != 0U ; counter--)
      {
        ...
        data |= (uint8_t)(OPENBL_ITF_USART_ReadByte(p_itf) & 0x00FFU);
        xor  ^= ((uint32_t)data & 0x00FFU);

        /* Only store data that fit in the buffer length */
        if (counter < (OPENBL_USART_RAM_BUFFER_SIZE / 2U))
        {
    ...
    OPENBL_ITF_USART_SendByte(p_itf, status);
  }
}
...
void OPENBL_USART_SpecialCommand(openbl_itf_obj_t *p_itf)
{
  ...
  if (OPENBL_USART_GetSpecialCmdOpCode(p_itf, OPENBL_SPECIAL_CMD, &op_code) == OPENBL_NACK_BYTE)
  {
    OPENBL_ITF_USART_SendAcknowledge(p_itf, OPENBL_NACK_BYTE);
  }
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        OPENBL_ITF_USART_SpecialCommandProcess(p_itf, special_cmd);
...
void OPENBL_USART_ExtendedSpecialCommand(openbl_itf_obj_t *p_itf)
{
  ...
  /* Get the command operation code */
  if (OPENBL_USART_GetSpecialCmdOpCode(p_itf, OPENBL_EXTENDED_SPECIAL_CMD, &op_code) == OPENBL_NACK_BYTE)
  {
    OPENBL_ITF_USART_SendAcknowledge(p_itf, OPENBL_NACK_BYTE);
  }
  else
...
/**
  ...
  * @param  address Pointer to the variable that will contain the received address.
  * @retval Returns NACK status in case of error else returns ACK status.
  */
static uint8_t OPENBL_USART_GetAddress(openbl_itf_obj_t *p_itf, openbl_mem_list_t *p_mem_list, uint32_t *p_address)
{
  uint8_t data[4] = {0U, 0U, 0U, 0U};
  uint8_t status;
  uint8_t xor;

  data[3] = OPENBL_ITF_USART_ReadByte(p_itf);
  data[2] = OPENBL_ITF_USART_ReadByte(p_itf);
  data[1] = OPENBL_ITF_USART_ReadByte(p_itf);
  data[0] = OPENBL_ITF_USART_ReadByte(p_itf);
                                                ...
                                                uint16_t *p_opcode)
{
  ...
  tmp_opcode[0] = OPENBL_ITF_USART_ReadByte(p_itf); /* Read the MSB byte */
  tmp_opcode[1] = OPENBL_ITF_USART_ReadByte(p_itf); /* Read the LSB byte */
